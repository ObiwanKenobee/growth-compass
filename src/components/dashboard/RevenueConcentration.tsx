...
import { revenueConcentrationData } from "@/data/dashboardData";

const RevenueConcentration = () => {
  const { topCustomers, metrics } = revenueConcentrationData;
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        <div className="p-3 rounded-lg bg-secondary/40 text-center">
          <p className={`text-lg font-bold font-mono ${metrics.top1Pct > 20 ? "text-danger" : metrics.top1Pct > 10 ? "text-warning" : "text-success"}`}>
            {metrics.top1Pct}%
          </p>
          <p className="text-xs text-muted-foreground mt-0.5">Top Client</p>
        </div>
        <div className="p-3 rounded-lg bg-secondary/40 text-center">
          <p className={`text-lg font-bold font-mono ${metrics.top5Pct > 50 ? "text-warning" : "text-success"}`}>
            {metrics.top5Pct}%
          </p>
          ...
          >
            ...
            <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
              <motion.div
                ...
                style={{
                  backgroundColor: cust.pct > 15 ? "hsl(0, 72%, 55%)" : cust.pct > 8 ? "hsl(38, 90%, 55%)" : "hsl(162, 63%, 45%)",
                }}
              />
            </div>
          </motion.div>
