...
import { useCustomerSegments } from "@/hooks/useDashboardData";

const CustomerSegments = () => {
  const { data: segments, isLoading } = useCustomerSegments();
  const maxLtv = segments ? Math.max(...segments.map(s => s.ltv)) : 1;
    ...
    >
      ...
      {isLoading ? (
        <div className="space-y-3">
          {[1,2,3].map(i => <div key={i} className="h-16 bg-secondary/40 rounded-lg animate-pulse" />)}
        </div>
      ) : (
