...
import { useExpansionRevenue } from "@/hooks/useDashboardData";

const ExpansionRevenue = () => {
  const { data: expData, isLoading } = useExpansionRevenue();

  const items = expData ? [
    { label: "Tier Upgrades", value: expData.upgrades, color: "bg-primary" },
    { label: "New Services", value: expData.newServices, color: "bg-info" },
    { label: "Geo Expansion", value: expData.geoExpansion, color: "bg-accent" },
  ] : [];
      ...
      {isLoading ? (
        ...
        <>
          <div className="flex items-end gap-3 mb-1">
            <span className="text-3xl font-bold font-display text-foreground">${(expData.total / 1000000).toFixed(1)}M</span>
            <div className="flex items-center gap-1 mb-1">
              <TrendingUp className="w-4 h-4 text-success" />
              <span className="text-sm font-mono text-success">+{expData.trend}%</span>
            </div>
          </div>
          <p className="text-xs text-muted-foreground mb-4">Revenue from existing customers expanding usage</p>
          <div className="h-28 mb-4">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={expData.history}>
                <defs>
