...
import { useCustomerHealth } from "@/hooks/useIntelligenceData";
...
const CustomerHealth = () => {
  const { data, isLoading } = useCustomerHealth();
  ...
  return (
    <>
      ...
      >
        ...
        <div className="mb-5">
          <div className="flex h-3 rounded-full overflow-hidden gap-0.5">
            <motion.div initial={{ width: 0 }} animate={{ width: `${summary.healthy}%` }} transition={{ duration: 0.8, delay: 0.6 }} className="bg-success rounded-l-full" />
            <motion.div initial={{ width: 0 }} animate={{ width: `${summary.atRisk}%` }} transition={{ duration: 0.8, delay: 0.7 }} className="bg-warning" />
            <motion.div initial={{ width: 0 }} animate={{ width: `${summary.churning}%` }} transition={{ duration: 0.8, delay: 0.8 }} className="bg-danger rounded-r-full" />
          </div>
        ...
        <div className="space-y-2">
          {accounts.map((acct: any, i: number) => {
            const cfg = statusConfig[acct.status as keyof typeof statusConfig] || statusConfig.healthy;
            const Icon = cfg.icon;
            return (
              <motion.div
                key={acct.name}
              ...
              >
                ...
                <div className="w-20 flex-shrink-0">
                  ...
                  <div className="h-1.5 rounded-full bg-secondary overflow-hidden">
                    <div className={`h-full rounded-full ${acct.score >= 70 ? "bg-success" : acct.score >= 40 ? "bg-warning" : "bg-danger"}`} style={{ width: `${acct.score}%` }} />
                  </div>
                </div>
                <span className={`text-xs px-2 py-0.5 rounded-full border font-mono ${cfg.bg} ${cfg.border} ${cfg.color}`}>{cfg.label}</span>
              </motion.div>
            );
          })}
        </div>
      ...
      <CustomerHealthModal account={selectedAccount} open={!!selectedAccount} onOpenChange={(open) => !open && setSelectedAccount(null)} />
    </>
  );
};
